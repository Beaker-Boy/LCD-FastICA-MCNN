...
- **多模型支持**: 内置 **MCNN、SimpleCNN、WDCNN、ResCNN** 四种神经网络架构 ⭐ *新增*
...
系统支持四种深度学习模型架构，可在 GUI 中灵活选择：

| 模型 | 结构特点 | 参数量 | 适用场景 |
|------|---------|--------|---------|
| **MCNN** | 多尺度自适应卷积（128×1, 64×1 宽核 + 5×1 多分支） | ~7.7M | 复杂特征提取，高精度需求 |
| **SimpleCNN** | 轻量化 3层卷积 (5×1) + 2层全连接 | ~62K | 快速验证，资源受限环境 |
| **WDCNN** | 宽深度网络 (128×1 宽核 + 5层 3×1 小核) | ~124K | 振动信号分析，抗噪性强 ⭐ |
| **ResCNN** | 残差学习 (4个残差块，5×1/7×1 交替) | ~1.1M | 深层网络，避免梯度消失 |

**推荐策略**：
- **学术研究**：对比所有四种模型，展示 WDCNN 在轴承故障诊断中的优势
- **工程应用**：优先使用 WDCNN（性能与效率平衡）
- **快速原型**：使用 SimpleCNN 验证数据质量
- **高性能需求**：使用 MCNN 或 ResCNN 追求最高准确率
...
3. **配置处理方法**:
   ...
   - **后处理**: 选择 无 / FastICA
   - **🎨 绘制中间产物图线** ⭐ *新增*: 勾选后将保存分解/分离过程中的分量图线至 `processed_data/ica_results/intermediate_plots/` 目录
...
process_signal_pipeline(
    ...
    plot_intermediate=True,  # 🎨 启用中间产物图线输出 ⭐ *新增*
    plot_save_dir='processed_data/ica_results/intermediate_plots'  # 图线保存目录
)

# === 基线方法：VMD-FastICA（对比用，不绘图）===
...
process_signal_pipeline(
    ...
    num_components=5,
    progress_callback=show_progress
    # plot_intermediate=False (默认，不绘图)
)
...
evaluate_single_model(
    ...
    data_dir='processed_data/tensor_dataset',
    save_dir='results/evaluation/lcd_fastica_results'
)

# 多模型对比（可选）
from src.evaluate import compare_models
model_paths = [
    'models/lcd_fastica_model.pth',
    'models/vmd_fastica_model.pth',
    'models/eemd_fastica_model.pth'
]
compare_models(
    model_paths=model_paths,
    data_dir='processed_data/tensor_dataset',
    output_dir='results/evaluation/method_comparison'
)
```
...
│   ├── evaluate.py            # 模型评估模块（混淆矩阵、t-SNE、分类报告）⭐ 新增
...
#### 6. 模型评估模块 (`evaluate.py`) ⭐ *新增*
- **功能**: 完整的模型性能评估与可视化
- **核心功能**:
  - **多指标评估**: 准确率、精确率、召回率、F1 分数（Macro/Weighted）
  - **混淆矩阵**: 可视化分类性能，支持归一化显示
  - **t-SNE 可视化**: 高维特征降维展示，直观观察类别分离度
  - **详细分类报告**: 每个类别的精确率、召回率、F1 分数
  - **概率统计**: 分析预测置信度分布
  - **对比实验**: 支持多个模型的批量对比
- **使用方式**:
  - GUI 操作：点击"评估已训练模型"或"对比多个模型"按钮
  - 命令行：`python src/evaluate.py --model_path models/best_model.pth --data_dir processed_data/tensor_dataset`
  - Python 调用：`from evaluate import evaluate_single_model; evaluate_single_model(model_path, data_dir, save_dir)`
...
- **src/evaluate.py**: ⭐ 模型评估模块（混淆矩阵、t-SNE、分类报告）

## 🔍 模型评估功能（新增）

### 功能概述
评估模块提供完整的模型性能分析工具，包括：
- ✅ **多指标评估**：准确率、精确率、召回率、F1 分数
- ✅ **混淆矩阵**：可视化分类效果，识别易混淆类别
- ✅ **t-SNE 可视化**：高维特征降维，直观展示类别分离度
- ✅ **详细分类报告**：每个类别的细分指标
- ✅ **概率统计**：预测置信度分析
- ✅ **对比实验**：多模型批量对比
...
2. **单模型评估**
   - 点击底部"评估已训练模型"按钮
   - 选择要评估的 `.pth` 模型文件
   - 确认数据集目录（默认：`processed_data/tensor_dataset`）
   - 点击"选择模型进行评估"
   - 查看结果：`results/evaluation/<模型名称>/`

3. **多模型对比**
   - 点击底部"对比多个模型"按钮
   - 使用 Ctrl/Cmd 键选择多个 `.pth` 文件
   - 确认数据集目录
   - 点击"选择多个模型进行对比"
   - 查看对比结果：`results/evaluation/comparison_<数量>_models/`
...
python src/evaluate.py \
    --model_path models/best_model.pth \
    --data_dir processed_data/tensor_dataset \
    --save_dir results/evaluation/my_evaluation
```

**多模型对比**：
```bash
python src/evaluate.py \
    --model_path models/lcd_fastica_model.pth \
    --compare models/vmd_fastica_model.pth models/eemd_fastica_model.pth \
    --data_dir processed_data/tensor_dataset \
    --save_dir results/evaluation/method_comparison
```
...
results = evaluate_single_model(
    model_path='models/best_model.pth',
    data_dir='processed_data/tensor_dataset',
    save_dir='results/evaluation/lcd_results'
)

print(f"准确率：{results['accuracy']:.4f}")
print(f"F1 分数：{results['f1_macro']:.4f}")
```
...
comparison_results = compare_models(
    model_paths=model_paths,
    data_dir='processed_data/tensor_dataset',
    output_dir='results/evaluation/comparison'
)
...
| 文件名 | 说明 |
|--------|------|
| `evaluation_results.json` | 完整评估指标（JSON 格式） |
| `confusion_matrix.png` | 混淆矩阵热图（归一化） |
| `tsne_visualization.png` | t-SNE 特征分布图 |
| `predictions_comparison.csv` | 每个样本的预测 vs 真实标签 |
| `probability_statistics.json` | 预测概率统计分析 |

**多模型对比时额外包含**：
| `model_comparison.json` | 多模型对比汇总表 |

### 评估指标解释

- **Accuracy (准确率)**: 正确预测的样本占总样本的比例
- **Precision-Macro (宏平均精确率)**: 各类别精确率的算术平均，平等对待每个类别
- **Precision-Weighted (加权精确率)**: 按类别样本数加权的精确率
- **Recall-Macro (宏平均召回率)**: 各类别召回率的算术平均
- **Recall-Weighted (加权召回率)**: 按类别样本数加权的召回率
- **F1-Macro (宏平均 F1)**: 各类别 F1 分数的算术平均
- **F1-Weighted (加权 F1)**: 按类别样本数加权的 F1 分数

**建议**：对于不平衡数据集，优先关注 Macro 指标；对于平衡数据集，Weighted 指标更具参考价值。
...
python src/evaluate.py \
    --model_path models/lcd_fastica_model.pth \
    --compare models/vmd_fastica_model.pth models/eemd_fastica_model.pth models/lmd_fastica_model.pth \
    --data_dir processed_data/tensor_dataset \
    --save_dir results/evaluation/main_experiment
```

**目的**：证明 LCD-FastICA 的综合优势

#### 2. 消融实验：验证 FastICA 的作用
```bash
python src/evaluate.py \
    --model_path models/lcd_fastica_model.pth \
    --compare models/lcd_only_model.pth \
    --data_dir processed_data/tensor_dataset \
    --save_dir results/evaluation/ablation_study
```

**目的**：量化 FastICA 对特征可分性的提升

#### 3. 鲁棒性实验：不同信噪比下的性能
对同一模型在不同噪声水平的测试集上进行评估：
```python
for snr in [-5, 0, 5, 10, 15]:
    test_data_path = f'processed_data/tensor_dataset_snr_{snr}dB'
    results = evaluate_single_model(
        model_path='models/lcd_fastica_model.pth',
        data_dir=test_data_path,
        save_dir=f'results/evaluation/robustness_snr_{snr}dB'
    )
    print(f"SNR={sn}dB: 准确率={results['accuracy']:.4f}")
```
...
### v2.1 (2026-04-14) 🎨 *可视化增强*
✨ **新增功能**:
- ✅ **中间产物图线输出**: 可选择是否绘制分解/分离过程中的分量图线
- ✅ **支持的绘图类型**: LCD、VMD、EEMD、LMD 分解分量图线，FastICA 独立成分图线
- ✅ **GUI 集成**: 添加"绘制中间产物图线"复选框，一键开启/关闭
- ✅ **自动保存**: 图线自动保存至 `processed_data/ica_results/intermediate_plots/` 目录
- ✅ **智能命名**: 图线文件按 `{文件名}_{方法名}.png` 格式自动命名

🔧 **技术实现**:
- 新增 `plot_intermediate_results()` 辅助函数
- 在 `process_signal_pipeline()` 中添加 `plot_intermediate` 和 `plot_save_dir` 参数
- 使用 matplotlib 创建多子图，包含原始信号和各分量对比
- 图形分辨率 150 DPI，自动关闭以避免内存泄漏
- 绘图失败仅记录警告，不影响主流程

📊 **使用场景**:
- **学术研究**: 直观展示信号分解效果，便于论文插图
- **教学演示**: 帮助学生理解各算法的分解原理
- **调试分析**: 快速定位信号处理中的问题
- **结果验证**: 目视检查分量的物理意义和合理性
