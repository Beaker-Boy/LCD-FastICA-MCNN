...
1. **GUI 交互模块 (`main_window.py`)**：
   - 提供图形界面，引导用户选择原始 `.npy` 文件、输入采样率、选择标签和训练模式，并支持将其加入批处理列表。
...
*   **阶段 3：流水线整合与特征生成**。将整个数据集送入 `LCD-FastICA` 流水线进行批量处理，将处理后的特征保存为 `.mat` 文件，进而构建张量并保存为 `.pt` 文件，以加速后续神经网络的训练。
*   **阶段 4：MCNN 网络搭建与初步训练**。基于 PyTorch 搭建多尺度 CNN 网络，接入处理好的特征数据，打通前向传播与反向传播的训练链路。
